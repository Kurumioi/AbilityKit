...
| P1 | Contract Blocker | `runtime-contracts`、`moba-xiaoqiao-unity` | 网络、DI、表现运行时、跨模块契约变化后，或需要确认 Unity 权威技能结果时 | 修复契约破坏，或同步更新设计文档和测试预期 |
...
| MOBA Unity EditMode | Unity batchmode EditMode 测试 | `moba-xiaoqiao-unity` | 小乔权威用例、Unity 侧技能结果和落盘产物 | 专项门禁可运行，但不是 PR 默认路径，适合保留为 P1 或 nightly 专项 |
