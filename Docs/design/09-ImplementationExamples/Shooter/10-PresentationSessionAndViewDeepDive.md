...
sequenceDiagram
    ...
    participant Link as ShooterCarrierNetworkLink
    ...
    loop each step
        ...
        Driver->>Link: PublishSnapshot(snapshot, elapsed)
        Driver->>Link: Advance(clockMs)
        Link-->>Client: Apply authoritative snapshot when due
    end
```
