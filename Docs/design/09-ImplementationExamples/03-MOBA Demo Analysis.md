...
- `MobaWorldBootstrapModule`：负责把 MOBA 的服务、系统、配置接入世界。
...
sequenceDiagram
    ...
    Service->>Link: Link(projectileId, actorId)
```
...
MOBA View Runtime 提供 `RemoteDrivenWorldRuntimeFactory`，用于远程对局或模拟权威帧驱动场景。创建时：
