...
6. `ShooterSmokeRunner` 做端到端烟测，验证推送、回放、丢帧、重连、晚加入等场景。
...
烟测的意义不是“跑通一次”，而是**证明战斗逻辑、网络同步、推送恢复、重连与晚加入是同一套协议下可互相验证的**。
