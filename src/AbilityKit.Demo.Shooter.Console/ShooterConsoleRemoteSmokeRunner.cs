...
internal sealed class ShooterConsoleRemoteSmokeRunner
{
    ...
    public int Run()
    {
        var projectPath = ResolveSmokeProjectPath();
        if (projectPath == null)
        {
        ...
        _output.Write(ConsoleOutputChannel.Sync, $"CASE name=\"Shooter Remote Gateway Smoke\" project=\"{Escape(NormalizePath(projectPath))}\" status=running");
    ...
    private static string? ResolveSmokeProjectPath()
    {
        var directory = new DirectoryInfo(Environment.CurrentDirectory);
        while (directory != null)
        {
            var candidate = Path.Combine(directory.FullName, SmokeProjectRelativePath);
        ...
        {
            var candidate = Path.Combine(baseDirectory.FullName, SmokeProjectRelativePath);
            if (File.Exists(candidate))
            {
                return Path.GetFullPath(candidate);
            }
    ...
    private void WriteProcessLine(ConsoleOutputChannel channel, string? line)
    {
        ...
        {
            _output.Write(ConsoleOutputChannel.Sync, $"CASE name=\"Shooter Remote Gateway Smoke\" status=completed detail=\"{Escape(normalized)}\"");
            return;
        }
