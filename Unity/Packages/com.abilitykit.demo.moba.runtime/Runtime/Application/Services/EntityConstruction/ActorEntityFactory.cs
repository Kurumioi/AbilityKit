...
namespace AbilityKit.Demo.Moba.Services.EntityConstruction
{
    ...
    {
        /* 创建一个基础 ActorEntity（不带任何组件），后续可通过 Builder 链式初始化。*/
    ...
    public readonly struct ActorEntityBuilder
    {
        ...
        public ActorEntityBuilder WithMotion()
        {
            ...
            {
                _entity.ReplaceMotion(pipeline, state, output, solver, policy, events, newInitialized: false, newHitTriggerRuntime: default);
            }
            else
            {
                _entity.AddMotion(pipeline, state, output, solver, policy, events, newInitialized: false, newHitTriggerRuntime: default);
            }
