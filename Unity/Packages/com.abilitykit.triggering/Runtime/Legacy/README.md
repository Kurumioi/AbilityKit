...
- `Executable/`：旧 Runtime/Executable 入口、旧配置转换器、旧示例与旧调度工厂。命名空间暂时保持不变，避免一次迁移破坏反序列化和外部引用。
...
- `ExecutableDsl`、各类 `*Builder` 与 `ScheduledExecutableBuilderExtensions` 均属于旧 Runtime/Executable 兼容入口；新代码不得继续使用这些类型，应迁移到 `Runtime/Plan`、`TriggerPlanBuilder`、`Plan/Executables` 与 `ActionScheduler`。
