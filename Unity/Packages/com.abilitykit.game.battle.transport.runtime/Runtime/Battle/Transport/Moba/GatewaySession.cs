...
namespace AbilityKit.Game.Battle.Transport.Moba
{
    ...
    /// <summary>
    /// Gateway 会话接口
    /// 抽象与 Gateway 服务器的连接和会话管理
    /// </summary>
    public interface IGatewaySession : IDisposable
    {
        ...
        Task SubmitFrameInputAsync(string roomId, ulong worldId, int frame,
            uint playerId, uint inputOpCode, byte[] inputPayload = null, CancellationToken cancellationToken = default);
    }

    /// <summary>
    /// Gateway 会话实现
    /// 管理与 Gateway 服务器的连接、认证和房间会话
    /// </summary>
    public sealed class GatewaySession : IGatewaySession
    {
        private readonly OrleansGatewayClient _client;
        ...
        {
            _client = new OrleansGatewayClient();
        ...
        public async Task<LoginResult> LoginAsGuestAsync(string guestId = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var payload = GatewayProtocol.EncodeGuestLoginReq(guestId);
                var response = await _client.SendRequestAsync(GatewayOpCodes.GuestLogin, payload, cancellationToken);
                var result = GatewayProtocol.DecodeGuestLoginResp(response);
        ...
        public async Task<RoomJoinResult> JoinRoomAsync(string roomId, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var payload = GatewayProtocol.EncodeJoinRoomReq(SessionToken, roomId);
                var response = await _client.SendRequestAsync(GatewayOpCodes.JoinRoom, payload, cancellationToken);
                var result = GatewayProtocol.DecodeJoinRoomResp(response);
        ...
        public async Task<RoomCreateResult> CreateRoomAsync(string roomId = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var payload = GatewayProtocol.EncodeCreateRoomReq(SessionToken, roomId ?? Guid.NewGuid().ToString());
                var response = await _client.SendRequestAsync(GatewayOpCodes.CreateRoom, payload, cancellationToken);
                var result = GatewayProtocol.DecodeCreateRoomResp(response);
        ...
        public async Task SubmitFrameInputAsync(string roomId, ulong worldId, int frame,
            uint playerId, uint inputOpCode, byte[] inputPayload = null, CancellationToken cancellationToken = default)
        {
            var payload = GatewayProtocol.EncodeSubmitFrameInput(roomId, worldId, frame, playerId, inputOpCode, inputPayload);
            await _client.SendServerPushAsync(GatewayOpCodes.SubmitFrameInput, payload, cancellationToken);
        }
